...
import { Search, Download, AlertCircle, CheckCircle2, Loader2, Info, Sparkles, Mail, MailX } from 'lucide-react';
...
export default function ApolloSearchPage() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    if (limit < 1) {
      setSearchError('Select a result count (5, 10, 25, 50, or 100).');
      return;
    }
    setSearching(true);
    setSearchError('');
  ...
  const handleEnrich = async (lead: ApolloLeadPreview) => {
    ...
    try {
      const res = await fetch('/api/apollo/enrich', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          apolloId: lead.apolloId, firstName: lead.firstName, lastName: lead.lastName,
          companyName: lead.companyName, website: lead.website,
          organizationDomain: lead.organizationDomain, linkedinUrl: lead.linkedinUrl, title: lead.title,
        }),
      });
      const data = await res.json();
      if (data.found && data.email) {
        setLeads((prev) => prev.map((l) =>
          l.apolloId === lead.apolloId ? { ...l, email: data.email, needsEnrichment: false, score: l.score + 10 } : l
        ));
      } else {
        setEnrichErrors((prev) => ({ ...prev, [lead.apolloId]: data.message ?? 'No email found after enrichment.' }));
      }
    } catch {
  ...
  // Apply email filter to visible leads
  const visibleLeads = emailFilter === 'email_only'
    ? leads.filter((l) => !l.needsEnrichment && !!l.email)
    : leads;
  ...
  const importableLeads = visibleLeads.filter(
      ...
      try {
        const res = await fetch('/api/apollo/import', {
          ...
          body: JSON.stringify({
            ...
            country: lead.country || country || 'Unknown', city: lead.city || '',
            category: lead.category || 'Apollo Lead', source: 'apollo',
            apolloId: lead.apolloId, title: lead.title, score: lead.score,
          }),
        });
        const data = await res.json();
        if (data.imported === true) { imported++; newlyImportedIds.add(lead.apolloId); }
        else if (data.duplicate === true) { skipped++; newlyImportedIds.add(lead.apolloId); }
        else { errors.push(`${lead.companyName}: ${data.error ?? 'Unknown error'}`); }
      } catch { errors.push(`${lead.companyName}: Network error`); }
    }
  ...
  const selectedLeads = visibleLeads.filter((l) => selected.has(l.apolloId));
  const allImportableSelected = importableLeads.length > 0 && selected.size === importableLeads.length;
  const needsEnrichmentCount = visibleLeads.filter((l) => l.needsEnrichment).length;
  const withEmailCount = leads.filter((l) => !l.needsEnrichment && l.email).length;
        ...
        <div className="mt-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-1.5 text-sm text-slate-600 cursor-pointer">
              <input type="radio" name="emailFilter" value="all"
                checked={emailFilter === 'all'} onChange={() => setEmailFilter('all')}
                className="text-emerald-600 focus:ring-emerald-500" />
              All Leads
            </label>
            <label className="flex items-center gap-1.5 text-sm text-slate-600 cursor-pointer">
              <input type="radio" name="emailFilter" value="email_only"
          ...
          {emailFilter === 'email_only' && leads.length > 0 && (
            <span className="text-xs text-slate-400 ml-2">
              {withEmailCount} of {leads.length} have email
            </span>
          )}
        </div>
      ...
      {showImportFailure && importResult && (
        <div className="mb-5 bg-rose-50 border border-rose-200 rounded-xl p-4 text-sm">
          <p className="font-semibold text-rose-700 mb-2 flex items-center gap-2">
            <AlertCircle size={16} /> Import Failed
          </p>
          {importResult.errors.length > 0
            ? <div className="text-xs text-rose-600">{importResult.errors.map((e, i) => <p key={i}>{e}</p>)}</div>
      ...
      {searched && visibleLeads.length === 0 && !searchError && (
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-sm">
          {leads.length > 0 && emailFilter === 'email_only'
            ? <><MailX size={20} className="mx-auto mb-2 opacity-40" /> No leads with email found. Try &ldquo;All Leads&rdquo; to see everyone, then enrich for emails.</>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
            <div className="flex items-center gap-3 text-sm text-slate-600">
              <span className="font-medium">{visibleLeads.length} leads</span>
              {visibleLeads.some((l) => l.isDuplicate) && (
                <span className="text-amber-600 font-medium">
                  · {visibleLeads.filter((l) => l.isDuplicate).length} already in DB
                </span>
              )}
              {importedIds.size > 0 && <span className="text-emerald-600 font-medium">· {importedIds.size} imported</span>}
              {needsEnrichmentCount > 0 && <span className="text-slate-500 font-medium">· {needsEnrichmentCount} need enrichment</span>}
              {selected.size > 0 && <span className="text-emerald-600 font-medium">· {selected.size} selected</span>}
            </div>
            <div className="flex items-center gap-2">
              {importableLeads.length === 0 && visibleLeads.length > 0 && (
              ...
              <button onClick={() => doImport(importableLeads)} disabled={importing || importableLeads.length === 0}
                ...
                <Download size={13} /> Import All ({importableLeads.length})
              </button>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                  <th className="px-4 py-3 text-left w-10">
                    <input type="checkbox" checked={allImportableSelected} onChange={toggleSelectAll} className="rounded" />
                  </th>
              ...
              <tbody className="divide-y divide-slate-50">
                {visibleLeads.map((lead) => {
                  ...
                  return (
                    <tr key={lead.apolloId}
                      className={`hover:bg-slate-50 transition-colors ${lead.isDuplicate || isImported ? 'opacity-50' : ''}`}>
                      <td className="px-4 py-3">
                        <input type="checkbox" checked={selected.has(lead.apolloId)}
                          disabled={!canSelect} onChange={() => canSelect && toggleSelect(lead.apolloId)}
                          className="rounded disabled:opacity-30" />
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-slate-800 truncate max-w-[160px]">{lead.companyName}</span>
                          {isImported && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-emerald-100 text-emerald-700 shrink-0">IMPORTED</span>}
                          {!isImported && lead.isDuplicate && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-100 text-amber-700 shrink-0">DUP</span>}
                        </div>
                        {lead.website && (
                      ...
                      <td className="px-4 py-3">
                        {lead.needsEnrichment ? (
                          <div className="flex flex-col gap-1">
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-100 text-amber-700 w-fit">Needs enrichment</span>
                            {enrichError
                              ? <span className="text-xs text-rose-500">{enrichError}</span>
                              : <button onClick={() => handleEnrich(lead)} disabled={isEnriching || lead.isDuplicate}
                                  className="flex items-center gap-1 text-xs text-emerald-600 hover:text-emerald-800 disabled:opacity-40 font-medium w-fit">
                                  {isEnriching ? <><Loader2 size={11} className="animate-spin" /> Enriching…</> : <><Sparkles size={11} /> Enrich Email</>}
                                </button>
                            }
                          </div>
                        ) : (
                          <div className="flex flex-col gap-0.5">
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-emerald-100 text-emerald-700 w-fit">
